...
-- Полностью совпадают 495 из 733
-- Полностью совпадают 495 из 740
 select count(*) n
  from "Бирюлёвский дендропарк"."№№ пл. сводка адресов"
...
-- Где-то чего-то нет 238 из 733
-- Где-то чего-то нет 245 из 740 
 select count(*) n
  from "Бирюлёвский дендропарк"."№№ пл. сводка адресов"
...
-- согласовано между OSM и экспликацией Дмитрия и нет в паспорте ОКН и на Википедии 190 из 238
-- согласовано между OSM и экспликацией Дмитрия и нет в паспорте ОКН и на Википедии 196 из 245 
 select count(*) n
  from "Бирюлёвский дендропарк"."№№ пл. сводка адресов"
 where not (wiki is not null and "ОКН"  is not null and "ОСМ"  is not null and "каталог Дмитрия" is not null)
...
INSERT INTO "Бирюлёвский дендропарк"."№№ пл. нет коллизий" ("Адрес","Причина") VALUES
     ...
     ('3×16а*', 'геометрически неверная (смещённая) часть посадки, не является отдельной ботанической единицей'),
...
INSERT INTO "Бирюлёвский дендропарк"."№№ пл. нет коллизий" ("Адрес","Причина") VALUES
     ('56×1а*','56-й участок не размечен'),
     ('56×1б*','56-й участок не размечен'),
     ('56×2а*','56-й участок не размечен'),
...
select * from "Бирюлёвский дендропарк"."№№ пл. нет коллизий"
where  "Адрес" not in (select  "Адрес" 
  from "Бирюлёвский дендропарк"."№№ пл. сводка адресов" 
);
